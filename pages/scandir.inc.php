...
/**
 * Recursiver Scan eines Verzeichnisses
 *
...
{
  function rexdev_scandir($source, $limit=0, $blacklist=array(), $whitelist=array(), $debug=false, &$result=array())
  {$fb = FirePHP::getInstance(true);
    ...
    if($debug)
    {
      $result['rawscan']['/'.str_replace($result['root'], '', $source)] = $rawscan;
    }
        ...
        { 
          ...
          {//$fb->log($dirscan,'$dirscan');
            ...
            {//$fb->log($dirscan,'$dirscan');
              ...
              //$ignoredcount = count($result['ignored']) + 1;
              //$result['ignored'][$ignoredcount] = str_replace($result['root'], '', $source.$item);$fb->groupEnd();
            }
          }
          $fb->groupEnd();
        }
    ...
    foreach($dirscan as $item)
    {
      ...
      if (is_dir($source.$item))
      {
        ...
        //$result['level'][$depth]['folders'][$i] = ltrim($source.$item,$result['root']);
        ...
        { //fb('LIMITED recursion');
          if($limit > $depth)
          {
            rexdev_scandir($source.$item.'/', $limit, $blacklist, $whitelist, $debug, $result);
          }
        }
        else
...
echo '<pre>';
highlight_string(
"<?php\n\n// rexdev_scandir(\$root,0,array('files','functions'),array('_*','*.php')) \n\n".
print_r(rexdev_scandir($root,0,array('_*'),array('_*','*.php'),true),true).
"\n?>\n"
);
echo '</pre>';

fb(rexdev_scandir($root,0,array('_*'),array('_*','*.php'),true));
